...
class DB:                        
    ...
    def update(self, id, product, price, comment):    
        # формируем запрос на обновление записи в БД
        self.cur.execute("UPDATE buy SET product=?, price=?, comment=? WHERE id=?", (product, price, comment, id,))
        # сохраняем изменения 
        self.conn.commit()
...
def on_closing(): 
    ...
    if messagebox.askokcancel("Уверены?", f"Закрыть {name}?"): 
...
l4 = Label(window, text="Всего затрат")
